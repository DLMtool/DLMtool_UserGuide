...
***Stock***, ***Fleet***, and ***Observation*** are all classes in the DLMtool, and, as the names suggest, contain the parameters for the fish stock, the fishing fleet, and the observations of the data, respectively.  

The *Stock*, *Fleet*, and *Observation* objects are contained within separate parameter tables, which are stitched together to form the *Operating Model* parameters.
...
**Note:** if you get an error message that says *Error: object 'Albacore' not found*, it suggests you have not loaded the DLMtool data correctly.  Go to [Unpack the data] to see how to do this.
...
### Observation Object
The final component for the *Operating Model* is the *Observation* object.  This object contains all the information relating to how the fishery information is generated inside the model.

Why do we need a *Observation* table?
...
Like the other two objects, there are a number of built-in *Observation* objects in the DLMtool.  
...
There are `r OnSlot` slots in *Observation* objects, each with information relating to the uncertainty of a data type.  
...
More information on the *Observation* object can be found by typing:
```{r, eval=FALSE}
class?Observation
```

## Existing Stock, Fleet, and Observation Objects
As mentioned earlier, there are several *Stock*, *Fleet*, and *Observation* objects that are built into the DLMtool.  A list of available *Stock*, *Fleet*, and *Observation* objects can be found using the `avail` function.  

```{r, include=FALSE}
nstocks <- length(avail("Stock"))
nfleet <- length(avail("Fleet"))
nobs <- length(avail("Observation"))
```
...
We can do the same thing for the `r nfleet` *Fleet* objects (`avail("Fleet")`) and the `r nobs` *Observation* objects (`avail("Observation")`).

## Creating the Operating Model Object
The built-in *Stock*, *Fleet*, and *Observation* objects means that it is straightforward to create an *Operating Model* and run the MSE using the DLMtool.  
...
Here we will create a Operating Model object called `myOM`, and use the `Blue_shark` *Stock* object, the `Generic_fleet` *Fleet* object, and the `Imprecise_Biased` *Observation* object:

```{r}
myOM <- new('OM', Blue_shark, Generic_fleet, Imprecise_Biased)
```
...
The remainder of this section will describe how to modify the existing *Stock*, *Fleet*, and *Observation* objects, as well as create new objects. 

## Customizing the Operating Model
The *Stock*, *Fleet*, and *Observation* objects that are built into the DLMtool can be used for running an MSE and exploring the output.  More often, however, users of the DLMtool will be interested in performing an MSE for their own particular fishery.  

To do this, the parameters of the *Operating Model* (or more specifically the *Stock*, *Fleet*, and *Observation* objects from which it is built) must be specified in such a way that reflects the characteristics of the fishery. 

There are a number of ways of doing this.

### Creating New Objects
One method is to create empty *Stock*, *Fleet*, and *Observation* objects and manually populate them.  This can be done using the `new` command:
```{r}
myStock <- new("Stock")
myFleet <- new("Fleet")
myObservation <- new("Observation")
```
This code creates new *Stock*, *Fleet*, and *Observation* objects that are empty and contain no information.
...
To import a *Observation* object from a CSV file you follow the same procedure:
```{r}
myObs <- new("Observation", "CSVs/myObs.csv")
```

Note, however, that the *Observation* object is large (`r length(slotNames(myObs))` parameters) and it is often easiest to modify an existing *Observation* object.

We can now stitch the three objects together into a new Operating Model:
```{r}
myOM <- new("OM", myStock, myFleet, myObs)
```
...
The *Stock*, *Fleet*, and *Observation* parameters are structured exactly the same as described above, but here they are in separate tabs of a single spreadsheet rather than separate CSV files.

The naming of the tabs for the Excel file is important.  The names of the three tabs relating to the *Stock*, *Fleet*, and *Observation* parameters must share a prefix followed by "Stock", "Fleet", or "Obs" respectively.  See the image below for an example.   
...
The *Fleet* and *Observation* objects can be modified in from existing objects in a similar way. First we copy existing objects, and then modify them to suit our needs:
...
```
Here we have modified a few parameters in the *Stock*, *Fleet*, and *Observation* objects and assumed that the rest of the values in built-in objects adequately describe our fishery.  We can now create an *Operating Model* object using the same method described previously:
```{r}
MyOM <- new("OM", MyStock, MyFleet, MyObs)
```
