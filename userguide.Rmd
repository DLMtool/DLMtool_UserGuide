...
link to github issues - pull requests, etc
...
If the MSE did not run successfully, repeat the previous steps, ensuring that you have the latest version of R and the DLMtool package. If still no success, please visit the DLMtool forum (XXX) or [contact us](http://www.datalimitedtoolkit.org/contact) with a description of the problem and we will try to help.
...
We will now look at how to combine objects of the four classes into an OM. For now we will work with the OM components that are built into DLMtool. In later sections of the user manual (XXX) we will cover how to build your own Stock, Fleet, Obs, and Imp objects that characterises your fishery.  
...
DLMtool is extensible and it is relatively straightforward to develop your own MPs and include them in the MSE. This is covered in XXX. 
...
More information on input control MPs can be found in XXX.
...
### Mixed MPs 
Mixed MPs return a combination of input and output controls. Currently there are only a few mixed MPs in DLMtool, and these were developed simply for demonstration purposes. They may not work very well! See XXX for more information on developing your own mixed MPs. And please [share](http://www.datalimitedtoolkit.org/contact) them with us, we'd love to add them to DLMtool!
...
DataInit("MyData", dir="ExampleData")
MyData <- XL2Data('MyData', dir="ExampleData")
```
...
<!--chapter:end:98-MSE_with_DLMtool.Rmd-->
