...
For example, to plot a *Stock* object (note that the figures may not appear correctly in this document, but are easy to re-size in R Studio):
```{r}
plot(Albacore)
```
...
The `Tplot`, `Tplot2` and `Tplot3` functions are part of a family of plotting functions that are fully customizable, and designed to work with Performance Metrics objects. See `?Tplot` and the [Developing Custom PMs] chapter for more information.
...
Parallel processing can increase the speed of running the MSE considerably. For example, although in this demonstration we are only running a low number of simulations (`nsim=48`), run time decreased from `r round(elapse1,0)` to `r round(elapse2,0)` seconds when using parallel processing on `r parallel::detectCores()*0.5` processors.
